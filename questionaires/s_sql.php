<?php
	...
	class s_questionare{
		...
		function student_num(){
			...
			if($res_student_num == '0')
			{
				echo $sql_student_num;
			}
		}

		function question1(){
			//统计选则了A选项的人数
			$sql_count_A = "select count(Q1) as count_A from s_questionare_tb where Q1='A'";
			$mysql = new mySql();
			$mysql->query($sql_count_A);
			$res_count_A = $mysql->getnext();
			if($res_count_A == '0')
			{
				echo $sql_count_A;
			}
			//统计选则了B选项的人数
			$sql_count_B = "select count(Q1) as count_B from s_questionare_tb where Q1='B'";
			$mysql = new mySql();
			$mysql->query($sql_count_B);
			$res_count_B = $mysql->getnext();
			if($res_count_B == '0')
			{
				echo $sql_count_B;
			}
