	...
	$("#find").click(function(){
		...
		xmlhttp.onreadystatechange = function(){
			...
			if(xmlhttp.readyState ==4 && xmlhttp.status == 200){
				...
				if(msg == '0'){
					document.getElementById('prompt').innerHTML="<span style='color:red'>0:用户不存在!</span>";
				}else if(msg == '2'){
					document.getElementById('prompt').innerHTML="<span style='color:red'>2:用户名或邮箱错误!</span>";
				}else if(msg == '1'){
					document.getElementById('prompt').innerHTML="<span style='color:red'>1:邮件已经发送，请及时查看！</span>";
					
					window.location.href="sendemail.php?username="+username+"&email="+email+"&loginid="+loginid;
				}else{
